...
export function normalizeWorkout(w) {
  ...
  // 고유 id: 앱이 주면 사용하고, 없으면 name+start로 생성.
...
// 여러 운동을 한 트랜잭션으로 저장. 저장된 러닝 수를 반환합니다.
export const saveWorkouts = db.transaction((workouts) => {
  let saved = 0;
...
export function getWorkouts({ limit = 500 } = {}) {
  return db
    .prepare(`SELECT * FROM workouts ORDER BY start_at DESC LIMIT ?`)
    .all(limit);
}
...
export function getSummary() {
  const rows = db
    .prepare(`SELECT * FROM workouts ORDER BY start_at ASC`)
    .all();
  ...
  // 주별 거리 + 주별 평균 체중 집계 (그 달의 N주차 기준: 예) 2026-05-2W)
  // 러닝이 있는 주와 체중을 잰 주를 합집합으로 묶어 같은 축에 비교합니다.
  ...
  // 월별 집계: 거리 합, 평균 페이스, (있으면) 평균 체중
  const monthMap = new Map(); // "YYYY-MM" -> { km, sec, wSum, wCount }
  const mbucket = (k) => {
...
// 주차 = ceil(일 / 7)  →  1~7일=1주차, 8~14일=2주차, ...
